...
function serviceMonthFull(halfNum) {
    return (halfNum == MONTH_HALF_FULL) ||(halfNum == MONTH_HALF_FULL_OVERTME) || (halfNum == MONTH_HALF_FULL_NIGHT) || (halfNum == MONTH_HALF_FULL_HOLIDAYS);
}
...
function serviceMonthFiller(halfNum, codeText, swithCallBack, keyNum, callbackValue) {
    //console.log('serviceMonthFiller', keyNum, codeText);
...
function getMonthHalfCount(newDoc, thisDoc, halfNum) {
    ...
    //console.log('COUNT', newObjVal, newObjCodeText);

    var counter = 0;

    function counterFiller(callbackValue) { counter++; }
    //текущее новое значение
    serviceMonthFiller(halfNum, newObjCodeText, counterFiller, secondKeyNum, 0);
        ...
        autoValue: function (newDoc) {
            if (this.isUpdate) {
                ...
                for (var i = 1; i <= endOfMonth; i++) {
                    //по календарю
                    var dateFrmt = doc.year + " " + doc.month + " " + i;
                    var momentDate = moment(dateFrmt, "YYYY MM DDZ");
                    ...
                    //по значению кода - relax или work
                    var dayWorkType = fSWT(doc['dt' + i]);
                    if (i == dtKeyNum) {
                       dayWorkType = fSWT(newObjSet['dt' + dtKeyNum]);
                    }

                    console.log(i, dtKeyNum, dayWorkType, isTodayHoliday, momentDate.day(), todayHoliday);

                    //todo календарь праздников и проверка заполнения поля другими значениями
                    if (dayWorkType == 'relax') {
                        if ((momentDate.day() == 0) || (momentDate.day() == 6) || isTodayHoliday) {
                            relaxDaysCount++;
                        }
                    }
                }
                console.log(relaxDaysCount);
...
var timecardSpecCodesSource = [
    ...
    { value:"spec31", codeNum:"31", codeText:"RP", code: 'РП', workType: 'relax', text:"РП (Время простоя по вине работодателя)"},

...
function getMonthHalfCount(newDoc, thisDoc, halfNum) {
    ...
    if ((halfNum == MONTH_HALF_FULL) || (halfNum == MONTH_HALF_FULL_OVERTME) || (halfNum == MONTH_HALF_FULL_NIGHT) || (halfNum == MONTH_HALF_FULL_HOLIDAYS)) {
        iStart = 1;
        iEnd = 31;
    }
...
function getMonthHalf(newDoc, thisDoc, halfNum) {
    ...
    for (var i = iStart; i <= iEnd; i++) {
        if (doc['d' + i] != '0') {
            if (i != secondKeyNum) {
                //vse
                if ((halfNum == MONTH_HALF_FULL) || (halfNum == MONTH_HALF_SECOND) || (halfNum == MONTH_HALF_FIRST)) {
                    var objType = fSWT(doc['dt' + i]);
                    if (objType == 'work') {
                        allTime += timestrToSec(doc['d' + i]);
                    }
                //sverhurochnye
                } else if (halfNum == MONTH_HALF_FULL_OVERTME) {
                    var objType = fSCT(doc['dt' + i]);
                    if (objType == 'C') {
                        allTime += timestrToSec(doc['d' + i]);
                    }
                    //ночные
                } else if (halfNum == MONTH_HALF_FULL_NIGHT) {
                    var objType = fSCT(doc['dt' + i]);
                    if (objType == 'N') {
                        allTime += timestrToSec(doc['d' + i]);
                    }
                    //праздники и выходные
                } else if (halfNum == MONTH_HALF_FULL_HOLIDAYS) {
                    var objType = fSCT(doc['dt' + i]);
                    if (objType == 'RP') {
                        allTime += timestrToSec(doc['d' + i]);
                    }
